<div>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-700 p-4 mb-4 flex flex-col sm:flex-row gap-3">
        <input
            ...
            class="flex-1 px-3 py-2 text-sm bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
        />
        <select
            wire:model.live="filterStatus"
            class="px-3 py-2 text-sm bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
        >
                    ...
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/30 transition-colors">
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-end gap-2">
                                @if($device->status !== 'approved')
                                    <button wire:click="approve({{ $device->id }})" wire:confirm="Approve this device?"
                                        class="text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg transition-colors">
                                        Approve
                                    </button>
                                @endif
                                @if($device->status !== 'blocked')
                                    <button wire:click="block({{ $device->id }})" wire:confirm="Block this device?"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-10 text-center text-sm text-zinc-400">
                            No devices from your students found
                        </td>
                    </tr>
                @endforelse
            </tbody>
