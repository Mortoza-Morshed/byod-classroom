            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div class="w-12 h-12 bg-zinc-100 dark:bg-zinc-700 rounded-xl flex items-center justify-center">
                        @if($existingDevice->device_type === 'laptop')
                            <flux:icon.computer-desktop class="w-6 h-6 text-zinc-600 dark:text-zinc-300" />
                        @elseif($existingDevice->device_type === 'tablet')
                            <flux:icon.device-tablet class="w-6 h-6 text-zinc-600 dark:text-zinc-300" />
                        @else
                            <flux:icon.device-phone-mobile class="w-6 h-6 text-zinc-600 dark:text-zinc-300" />
                        @endif
                    </div>
                    <div>
            ...
            <form wire:submit="register" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        @foreach(['laptop' => 'Laptop', 'tablet' => 'Tablet', 'phone' => 'Phone'] as $value => $label)
                            <label class="relative cursor-pointer">
                                <input
                                    ...
                                    class="sr-only peer"
                                />
                                <div class="flex flex-col items-center gap-2 p-3 bg-zinc-50 dark:bg-zinc-900 border-2 border-zinc-200 dark:border-zinc-700 rounded-lg peer-checked:border-sky-500 peer-checked:bg-sky-50 dark:peer-checked:bg-sky-900/20 transition">
                                    @if($value === 'laptop')
                                        <flux:icon.computer-desktop class="w-5 h-5 text-zinc-500 peer-checked:text-sky-600" />
                                    @elseif($value === 'tablet')
                                        <flux:icon.device-tablet class="w-5 h-5 text-zinc-500" />
                                    @else
                                        <flux:icon.device-phone-mobile class="w-5 h-5 text-zinc-500" />
                                    @endif
